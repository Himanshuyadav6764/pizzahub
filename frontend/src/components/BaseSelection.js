...
const BaseSelection = ({ bases, selection, handleSelectionChange, handleStepChange }) => {
    const onRadioChange = (event) => {
        console.log('Base selected:', event.target.value); // Debug log
        handleSelectionChange('base')(event);
        // Auto move to next step after base selection
        setTimeout(() => {
            if (handleStepChange) {
                handleStepChange('sauce');
            }
        }, 300);
    };

    console.log('Bases available:', bases); // Debug log
    console.log('Current selection:', selection); // Debug log
